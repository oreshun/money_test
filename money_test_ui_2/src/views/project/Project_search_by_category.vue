...
const categroy_id = ref(route.params.id); // 动态路由参数
//获取当前类别的项目
const projects = ref([])//存储项目
const get_projects_by_category = async () => {
  let response = await axios.post('/project_operate/categroy_name',{
    category_name:categroy_name.value,
  },{
    headers: {
      Authorization: token,
    }
...
//获取当前类别名称
const categroy_name = ref('')
const get_categroy_name = async () => {
  let response = await axios.get('/project_operate/categroy_name?category_id='+Number(categroy_id.value),{
    headers: {
      Authorization: token
    }
  });
  categroy_name.value = response.data.category.category_name;
  //设置初始值
  el_option_value.value = response.data.category.category_id;
}
// 监听路由参数变化
watch(() => route.params.id, async (newId) => {
  categroy_id.value = newId; // 更新当前类别 ID
  await get_categroy_name(); // 获取新的类别名称
  await get_projects_by_category(); // 获取新的项目列表
});
onMounted(async ()=>{
  //调用
  await get_categroy_name();
  fetchCategories();
...
const search_project_by_time = async () =>{
  ...
  //传递信息进行操作
  let response = await axios.post('/project_operate/search-ByTime', {
        start_time: timeRange.length ? timeRange[0] : null,
        end_time: timeRange.length ? timeRange[1] : null,
        category:categroy_name.value,
    ...
    <el-container>
      ...
      <el-main>
        <div class="main-title">
          <h3 class="fw-bolder">{{categroy_name}} · 众筹项目</h3>
        </div>
...
.fw-bolder{
  ...
  text-shadow: 1px 1px 4px rgba(0, 0, 0, 0.6); /* 添加文字阴影 */
}
